...
/**
 * Partner lockup, files under `public/logos/`.
 * Both vendors ship artwork for light backgrounds, so the app points at `-on-dark`
 * variants: SU YAPI's white surround repainted black (lettering left white), and
 * Para Sidara's near-black (#231f20) wordmark lightened. Only those fills changed —
 * the untouched originals stay alongside as `su-yapi.webp` / `para-sidara.svg`.
 */
const PARTNER_LOGOS = [
  { src: '/logos/su-yapi-on-dark.webp', alt: 'SU YAPI Engineering & Consulting', width: 1280, height: 203 },
  { src: '/logos/para-sidara-on-dark.svg', alt: 'Para Sidara', width: 240, height: 31 },
] as const;
...
function seedHistory(base: number, amplitude: number, decimals: number): number[] {
  return Array.from({ length: HISTORY_POINTS }, (_, i) =>
    parseFloat((base + Math.sin(i * 0.7) * amplitude + Math.cos(i * 0.31) * amplitude * 0.4).toFixed(decimals))
  );
}
...
const SEED_LOG: LogEntry[] = [
  { id: -1, time: '09:42:10', level: 'success', text: 'Automated integrity self-test passed — 6/6 structural nodes reporting.' },
  { id: -2, time: '09:41:55', level: 'info', text: 'Telemetry uplink established. Physics model synchronised to twin.' },
];
...
export default function DamMonitoringDashboard() {
  ...
  const sensors = useMemo(() => readSensors({ surge, pga, spillwayGate }), [surge, pga, spillwayGate]);
  const risk = useMemo(() => computeRisk({ surge, pga, spillwayGate }), [surge, pga, spillwayGate]);
  ...
  const [histories, setHistories] = useState<Record<string, number[]>>(() =>
    Object.fromEntries(SENSOR_META.map((m) => [m.id, seedHistory(m.base, m.base * 0.004, m.decimals)]))
  );
  const [riskHistory, setRiskHistory] = useState<number[]>(() => seedHistory(12, 0.5, 1));
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setHistories((prev) => {
        ...
        for (const s of cur) {
          const jitter = (Math.random() - 0.5) * Math.max(0.05, s.value * 0.0015);
          next[s.id] = [...(prev[s.id] ?? []).slice(1), parseFloat((s.value + jitter).toFixed(s.decimals))];
        }
        return next;
      });
      setRiskHistory((prev) => [...prev.slice(1), parseFloat(curRisk.score.toFixed(1))]);
    }, TICK_MS);
  ...
  const pushLog = useCallback((level: EventLevel, text: string) => {
    ...
    setLog((prev) => [{ id: logId.current, time, level, text }, ...prev].slice(0, 40));
  }, []);
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      while (fired < activeScenario.events.length && activeScenario.events[fired].at <= p) {
  ...
  const stopScenario = () => {
    setActiveScenarioId(null);
    setScenarioProgress(0);
    pushLog('info', 'Scenario aborted by operator. Inputs held at current values.');
  };
    ...
    <div className="min-h-screen flex flex-col bg-[#0b0f19] text-gray-100 font-sans">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">Dam Safety Digital Monitoring &amp; Predictive Intelligence</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 text-xs">
          <div className="glass-panel px-3.5 py-1.5 rounded-full flex items-center gap-2">
            ...
            <span>
              ...
              <strong className={STATUS_TEXT[risk.status]}>
                {risk.status === 'critical' ? 'CRITICAL ALERT' : risk.status === 'warning' ? 'ELEVATED RISK' : 'OPERATIONAL'}
              </strong>
            </span>
          </div>
          ...
          <div className="glass-panel px-3.5 py-1.5 rounded-full flex items-center gap-2">
            ...
            <span>
              ...
              {alarmCount > 0 && <strong className="text-amber-400 ml-1.5 tabular-nums">· {alarmCount} in alarm</strong>}
            </span>
          </div>
        ...
        <div className="glass-panel p-5 bg-gradient-to-r from-[#00e5ff]/10 via-purple-500/10 to-transparent border-[#00e5ff]/30 flex flex-wrap items-center justify-between gap-4">
          <div>
            <span className="text-xs font-semibold uppercase tracking-wider text-[#00e5ff] flex items-center gap-1.5">
              <Zap className="w-3.5 h-3.5" /> Investor &amp; Executive Pitch Metrics
            </span>
            <h2 className="text-base font-bold text-white mt-1">Digital Twin Safety Management &amp; Predictive ROI</h2>
            <p className="text-xs text-gray-400 mt-0.5">
              Real-time telemetry, 3D structural twin, &amp; physics-AI risk predictions benchmarked against ICOLD safety guidelines.
            </p>
          </div>
          ...
          <div className="flex items-center gap-8">
            {[
              ...
              <div key={m.l} className="text-center">
                <div className={`text-xl font-extrabold font-mono tabular-nums ${m.c}`}>{m.v}</div>
      ...
      <main className="grid grid-cols-1 lg:grid-cols-12 gap-5 p-6 flex-1">
        ...
        <section className="lg:col-span-3 flex flex-col gap-5">
          <div className="glass-panel p-4 flex flex-col gap-4">
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
              <h2 className="text-sm font-semibold flex items-center gap-2 text-white">
                <Activity className="w-4 h-4 text-[#00e5ff]" /> Live Telemetry Overview
              </h2>
            ...
            <div className="grid grid-cols-2 gap-2.5">
              {['WL-01', 'P-01', 'SF-02', 'INC-04'].map((id) => {
                ...
                return (
                  <div key={id} className="bg-white/[0.03] border border-white/10 p-3 rounded-lg flex flex-col gap-1">
                    <span className="text-[11px] text-gray-400">{s.type === 'Water Level Radar' ? 'Reservoir Level' : s.type === 'Piezometer' ? 'Pore Water Press.' : s.type === 'Seepage Flow Gauge' ? 'Seepage Flow' : 'Crest Displacement'}</span>
                    <span className={`text-base font-bold font-mono tabular-nums ${STATUS_TEXT[s.status]}`}>
                      {s.value} {s.unit}
                    </span>
                    ...
                    <span className="text-[10px] text-gray-500 tabular-nums">
                      {s.status === 'critical' ? 'Limit exceeded' : s.status === 'warning' ? 'Advisory band' : 'In envelope'} · {s.critAt}
                      {' '}
                      {s.unit}
                    </span>
                  </div>
            ...
            <div className="flex flex-col gap-2 mt-1">
              <h3 className="text-xs font-medium text-gray-400">Active Structural Sensors ({sensors.length})</h3>
                ...
                {sensors.map((sensor) => (
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <div className="text-[10px] text-gray-400 truncate">{sensor.location}</div>
                    </div>
                    <div className="text-right shrink-0 pl-2">
                      <div className={`text-xs font-mono font-bold tabular-nums ${STATUS_TEXT[sensor.status]}`}>
                        {sensor.value} {sensor.unit}
                      </div>
                      <div className="text-[9px] text-gray-500 tabular-nums">Limit: {sensor.critAt}</div>
                    </div>
                  </button>
                ))}
              </div>
          ...
          <div className="glass-panel p-4 flex flex-col gap-3 flex-1">
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
              <h2 className="text-sm font-semibold flex items-center gap-2 text-white">
                <ListFilter className="w-4 h-4 text-[#00e5ff]" /> Event &amp; Alarm Log
              </h2>
            ...
            <div className="space-y-2 overflow-y-auto max-h-[300px] custom-scrollbar pr-1">
              {log.map((e) => (
                <div key={e.id} className="flex gap-2 text-[11px] leading-snug">
                  <span className={`w-1.5 h-1.5 rounded-full mt-1.5 shrink-0 ${LEVEL_STYLES[e.level].dot}`} />
                  <div className="min-w-0">
                    <span className="font-mono text-gray-500 tabular-nums mr-1.5">{e.time}</span>
        ...
        <section className="lg:col-span-6 flex flex-col gap-5">
            ...
            {activeScenario && (
              <div className="mt-2.5 px-1">
                <div className="flex justify-between text-[10px] text-gray-400 mb-1">
                  <span className="text-[#00e5ff] font-medium">{activeScenario.name} — {activeScenario.blurb}</span>
                  <span className="font-mono tabular-nums">{Math.round(scenarioProgress * 100)}%</span>
                </div>
          ...
          <div className="glass-panel p-3">
            <div className="flex items-center justify-between px-2 pb-2">
              <h2 className="text-sm font-semibold flex items-center gap-2 text-white">
                <Building2 className="w-4 h-4 text-[#00e5ff]" /> 3D Digital Twin Visualizer
              </h2>
          ...
          <div className="glass-panel p-4">
            <div className="flex items-center justify-between pb-3">
              <h2 className="text-sm font-semibold flex items-center gap-2 text-white">
                <TrendingUp className="w-4 h-4 text-[#00e5ff]" /> Sensor Data Streams
              </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                ...
                <TelemetryChart
                  ...
                  thresholds={[
                    { value: selectedSensor.warnAt, label: 'Advisory', color: '#f59e0b' },
                    { value: selectedSensor.critAt, label: 'Limit', color: '#ef4444' },
                  ]}
                />
              </div>
              <div>
                <h4 className="text-xs text-gray-300 mb-1.5 font-medium">Predicted Risk Score · Physics-AI model</h4>
                <TelemetryChart
                  title="Risk Score"
        ...
        <section className="lg:col-span-3 glass-panel p-4 flex flex-col gap-4">
          <div className="flex items-center justify-between border-b border-white/10 pb-3">
            <h2 className="text-sm font-semibold flex items-center gap-2 text-white">
              <Sliders className="w-4 h-4 text-purple-400" /> AI What-If Simulator
            </h2>
          ...
          <RiskGauge
            score={risk.score}
            drivers={[
              { label: 'Hydrostatic', value: risk.hydrostatic, color: '#17a5c9' },
          ...
          <div className="space-y-4">
            {[
              { label: 'Reservoir Surge (Flood Surge)', value: surge, set: setSurge, min: 0, max: 15, step: 0.5, suffix: ' m', prefix: '+', accent: 'accent-[#00e5ff]', text: 'text-[#00e5ff]', digits: 1 },
              { label: 'Seismic Ground Accel. (PGA)', value: pga, set: setPga, min: 0, max: 0.45, step: 0.01, suffix: ' g', prefix: '', accent: 'accent-purple-400', text: 'text-purple-400', digits: 2 },
              { label: 'Spillway Gate Opening', value: spillwayGate, set: setSpillwayGate, min: 10, max: 100, step: 5, suffix: '%', prefix: '', accent: 'accent-emerald-400', text: 'text-emerald-400', digits: 0 },
            ].map((s) => (
              <div key={s.label}>
            ...
            {simLocked && (
              <p className="text-[10px] text-gray-500 -mt-1">Inputs are driven by the running scenario. Press Stop to regain manual control.</p>
            )}
          </div>
      ...
      <footer className="mt-auto px-6 py-3 border-t border-white/10 bg-[#0b0f19]/90">
        <p className="text-[11px] text-gray-500">
          {FACILITY_NAME} · demonstration dataset — figures are simulated, not live field telemetry.
        </p>
      </footer>
    </div>
  );
